...
class DataTransformation:
    ...
    def initiate_data_transformation(self,train_path,test_path):
        ...
        try:
            ...
            print ("x_train_transf_preprocessed_df shape before cfs:", x_train_transf_preprocessed_df.shape)
            ...
            cor = x_train_transf_preprocessed_df.corr()

            logging.info("CFS started")
            ...
            logging.info("CFS performed")

            corr_features = correlation (x_train_transf_preprocessed_df, 0.7)
            len (set(corr_features))
            # print ('correlated features length:', len(set(corr_features)))
            print ("corrlated features are:", corr_features)

            logging.info("Correlated features obtained")

            x_train_transf_preprocessed_df_cfs = x_train_transf_preprocessed_df.drop (corr_features, axis =1)

            logging.info("correlated features dropped from x_train_transf_preprocessed_df")
            ...
            x_test_transf_preprocessed_df_cfs = x_test_transf_preprocessed_df.drop(corr_features, axis=1)

            logging.info("correlated features dropped from x_test_transf")

            print ("x_train columns after dropping correlated features:", x_train_transf_preprocessed_df_cfs.columns)

            # print ("x_train_transf_cfs shape after dropping correlated features::", x_train_transf_preprocessed_df_cfs.shape)

            print ("x_test_transf_be columns after dropping correlated features::", x_test_transf_preprocessed_df_cfs.columns)

            # print ("x_test_transf_be shape after dropping correlated features::", x_test_transf_preprocessed_df_cfs.shape)

            train_arr = np.c_[np.array(x_train_transf_preprocessed_df_cfs), np.array(y_train_transf)]
            
            logging.info("Combined the input features and target feature of the train set as an array.")
            
            test_arr = np.c_[np.array(x_test_transf_preprocessed_df_cfs), np.array(y_test_transf)]
